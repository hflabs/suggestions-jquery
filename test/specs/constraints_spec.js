describe("Address constraints", function() {
    ...
    it("Should have `locations` parameter in request with several fias params", function() {
        ...
        expect(this.server.requests[0].requestBody).toContain(
            '"locations":[{"region_fias_id":"44","area_fias_id":"55"}]'
        );
    });
    ...
    it("Should have `locations` parameter in request with only acceptable fields", function() {
        ...
        expect(this.server.requests[0].requestBody).toContain(
            '"locations":[{"country":"россия","region":"москва","city":"москва"}]'
        );
    });
    ...
    it("Should have `locations` parameter in request if constraints specified as array of objects", function() {
        ...
        expect(this.server.requests[0].requestBody).toContain(
            '"locations":[{"region":"Москва"},{"kladr_id":"6500000000000"}]'
        );
    });
    ...
    it("Should show label for x_type_full constraint, which is build from locations", function() {
        ...
        expect($items.first().text()).toEqual(
            "region, area, city, city_district, settlement, street"
        );
    });
    ...
    it("Should set unrestricted suggestion value", function() {
        ...
        expect(this.instance.suggestions[0]).toEqual({
            ...
            unrestricted_value:
                "обл Ростовская, г Ростов-на-Дону, ул Буквенная, д 20",
            data: null
        });
    });
    ...
    describe("in cooperation with other control", function() {
        ...
        it("Should use parent data as a constraint in child", function() {
            ...
            expect(this.server.requests[0].requestBody).toContain(
                '"locations":[{"kladr_id":"7800000000000"}]'
            );
            expect(this.server.requests[0].requestBody).toContain(
                '"restrict_value":true'
            );
        ...
        it("Should spread data to all parents", function() {
            ...
            this.input.value =
                "г Узловая, поселок Брусянский, ул Строителей, д 1-бара";
    ...
    describe("can restrict values", function() {
        ...
        it("one constraint (region)", function() {
            ...
            expect(
                ...
                "Узловский р-н, г Узловая, поселок Брусянский, ул Строителей, д 1-бара"
            );
        });
        ...
        it("one constraint (region by region_fias_id)", function() {
            ...
            var value = this.instance.getSuggestionValue(suggestion, {
                hasBeenEnriched: true
            });
                    ...
                    this.instance.getSuggestionValue(
                        {
                            data: {
                                ...
                                street: "Турчанинов",
                                street_fias_id:
                                    "0f7981e6-65c6-4513-b771-f5db3bfafe60",
                    ...
                    this.instance.getSuggestionValue(
                        {
                            data: {
                                city: "Коломна",
                                city_fias_id:
                                    "b367fb03-29f9-4dac-8d85-01595cfb6ad9",
