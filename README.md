...
В последующих запросах этот объект будет передаваться на сервер, и сервер будет учитывать его при ранжировании найденных подсказок.
...
Также, есть возможность задать местоположение вручную:
...
Можно задать местоположение для всех экземпляров плагина:
    ...
    $.Suggestions.defaultOptions.geoLocation = {
         kladr_id: ...,
         region: ...
    };
...
Пример:
    ...
    $('#address).suggestions('getGeoLocation')
        ...
        .fail(function(){
            // Функция будет вызвана, если местоположение определенить не удалось.
        })
