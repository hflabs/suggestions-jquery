...
* `$(selector).suggestions(options);`
    ...
    * `options`: Объект, определяющий начальные параметры работы плагина.
        ...
        * `useDadata`: Флаг, предписывающий использовть сервис dadata.ru для обогащения подсказок. По умолчанию: `true`, но требует наличия опций `token` и `type`. Обращение производится в случае выбора подсказки из списка (если определен обработчик `onSelect`). В `onSelect` будет передан агрумент `suggestion`, дополненный данными от dadata.ru
