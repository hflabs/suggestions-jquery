...
* `$(selector).suggestions(options);`
    * Включает плагин для выбранных полей ввода.
    * `options`: Объект, определяющий начальные параметры работы плагина.
        ...
        * `token`: Строка, передаваемая в запросе в целях авторизации.
...
* `enable`: фключает плагин, если он был деактивирован.
* `hide`: скрывает список с подсказками.
* `dispose`: уничтожает экземпляр плагина.

Есть два способа обращения к методам плагина. Первый состоит в передаче названия метода как строкового аргумента, за которым могут следовать иные необходимые аргументы:
...
Сервера должен передавать следующий объект в формате JSON:
...
Атрибут `data` может быть любого типа. Его значение будет передаваться в функции formatResults и onSelect. Если этот атрибут не используется, то в response.suggestions можно передавать просто массив строк:
