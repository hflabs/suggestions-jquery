...
* `$(selector).suggestions(options);`
    ...
    * `options`: Объект, определяющий начальные параметры работы плагина.
        ...
        * `constraints`: Ограничения, сужающие область поиска. Может быть как объектом со структурой `constraint` (см. ниже), так и массивом таких объектов.
...
Поле `restrictions` задает непосредственно условия поиска. Может содержать: `kladr_id`, `okato`, `postal_code`, `region`, `area`, `city` и `settlement`.
